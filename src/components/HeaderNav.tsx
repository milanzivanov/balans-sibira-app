    ...
    <>
      ...
      <ul className="hidden md:flex items-center gap-4 font-medium text-slate-700 dark:text-slate-100">
        {navLinks.map((link) => (
          <li key={link.href}>
            <Link
              className={`hover:underline ${pathname === link.href ? "underline text-blue-300" : ""} transition-colors`}
              href={link.href}
            >
              {link.label}
            </Link>
      ...
      {isMobileMenuOpen && (
        <>
          ...
          />
          <nav className="md:hidden fixed inset-0 bg-gray-50 dark:bg-gray-800 z-40 pt-17 sm:pt-[99.1px]" role="navigation" aria-label="Mobile Navigation">
